...
def get_existing_model(model, column_name, column_value, **kwargs):
    """
    Gets model instance from the database by a crertain field.
    ...
    Returns:
        ...
        error: Else expection is raised with appropriate message.
    """
    model_instance = model.objects.filter(**{column_name: column_value})
    if len(model_instance) > 0:
        errors.custom_message(
            f'{model.__name__} with {column_value} already exist'
        )
